...
public class ReporteGananciasDAO {
    ...
    String sql = """
        SELECT
            ...
            COALESCE(boletos.ingresos_boletos, 0) AS ingresos_boletos,

            COALESCE(alquileres.ingresos_alquileres, 0) AS ingresos_alquileres,

            COALESCE(boletos.ingresos_boletos, 0)
            + COALESCE(alquileres.ingresos_alquileres, 0)
            AS total_ingresos,
        ...
        LEFT JOIN (
            SELECT
                bus.codigo_sucursal,
                SUM(b.precio) AS ingresos_boletos
            FROM bus
            INNER JOIN viaje v
            ...
            ON boletos.codigo_sucursal = s.codigo_sucursal

        LEFT JOIN (
            SELECT
                bus.codigo_sucursal,
                SUM(pa.monto) AS ingresos_alquileres
            FROM bus
            INNER JOIN viaje v
                ON v.placa_bus = bus.placa
            ...
            INNER JOIN pago_alquiler pa
                ON pa.codigo_alquiler = a.codigo_alquiler
                AND pa.fecha_pago BETWEEN ? AND ?
            GROUP BY bus.codigo_sucursal
        ) alquileres
            ON alquileres.codigo_sucursal = s.codigo_sucursal
