...
public class SucursalDAO {
    ...
    public boolean activar(String codigoSucursal) {
        String sql = """
                 UPDATE sucursal
                 SET estado = TRUE
                 WHERE codigo_sucursal = ?
        ...
        try (Connection conexion = Conexion.getConnection(); PreparedStatement ps = conexion.prepareStatement(sql)) {
            ...
            System.out.println("Error al activar sucursal: " + e.getMessage());
            return false;
        }
    }
    ...
    public List<Sucursal> listar() {
        ...
        String sql = """
                 SELECT codigo_sucursal, nombre, direccion, telefono,
                        municipio, departamento, latitud, longitud, estado
                 FROM sucursal
                 """;

        List<Sucursal> sucursales = new ArrayList<>();

        try (Connection conexion = Conexion.getConnection(); PreparedStatement ps = conexion.prepareStatement(sql); var rs = ps.executeQuery()) {
            while (rs.next()) {
